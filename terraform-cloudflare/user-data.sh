...
# Download Cloudflare IP ranges for Nginx real IP configuration
sudo mkdir -p /etc/nginx/cloudflare
sudo curl -s https://www.cloudflare.com/ips-v4 -o /etc/nginx/cloudflare/ips-v4.txt
sudo curl -s https://www.cloudflare.com/ips-v6 -o /etc/nginx/cloudflare/ips-v6.txt

# Create Nginx configuration with Cloudflare support
%{ if domain_name != "" ~}
...
sudo tee /etc/nginx/conf.d/${app_name}.conf > /dev/null <<NGINX_EOF
upstream nextjs {
...
server {
    ...
    location / {
        ...
        # Cloudflare headers - preserve real visitor IP
...
echo "Cloudflare configuration enabled" >> /var/log/user-data.log
